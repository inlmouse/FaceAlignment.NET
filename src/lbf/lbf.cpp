...
void LbfCascador::DumpTrainModel(int stage) {
    ...
    char buff[50];
    sprintf(buff, "C:\\Research\\face-alignment\\model\\tmp_stage_%d.model", stage);
...
void LbfCascador::ResumeTrainModel(int start_from) {
    ...
    double *ptr;
    for (int k = 0; k < start_from; k++) {
        sprintf(buff, "C:\\Research\\face-alignment\\model\\tmp_stage_%d.model", k);
        fin = fopen(buff, "rb");
        assert(fin);
...
std::vector<std::vector<double>> LbfCascador::Predict(Mat &img, BBox &bbox, bool isact) {
	BBox temp;
